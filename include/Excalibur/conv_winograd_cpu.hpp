...
namespace glasssix
{
	namespace excalibur
	{
		...
		public:
			...
			void Forward_F23(const std::shared_ptr<memory::tensor<float>>& bottom, std::shared_ptr<memory::tensor<float>>& top);

#ifdef _MSC_VER
			void Forward_F43(const std::shared_ptr<memory::tensor<float>>& bottom, std::shared_ptr<memory::tensor<float>>& top);
#endif
