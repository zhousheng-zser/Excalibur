static void conv1x1s1_sgemm_neon(
	...
	const std::shared_ptr<glasssix::memory::tensor<float>>& bias)
{
	...
	const float* kernel_sgemm_data = kernel_sgemm->cpu_data();
	...
	{
		const float* bottom_im2col_data = bottom->cpu_data() + num_i * bottom->count(1, 4);
		float* top_data = top->mutable_cpu_data() + num_i * top->count(1, 4);
		im2col_sgemm_neon(bottom_im2col_data, inch, 1, size, top_data, outch, outh, outw, kernel_sgemm_data, kernel_sgemm_cstep, bias_data);
	}
}

static void conv1x1s1_neon(const std::shared_ptr<glasssix::memory::tensor<float>>& bottom,
	std::shared_ptr<glasssix::memory::tensor<float>>& top, const std::shared_ptr<glasssix::memory::tensor<float>>& kernel,
	const std::shared_ptr<glasssix::memory::tensor<float>>& bias)
{
	int num = bottom->num();
	int inch = bottom->channels();
...
#pragma omp parallel for num_threads(2) 
		...
		for (int pp = 0; pp < nn_outch; pp++)
		{
			...
			for (; q + 7 < inch; q += 8)
			{
				...
				const float* kernel0 = kernel_data + p * inch + q;
				const float* kernel1 = kernel_data + (p + 1) * inch + q;
				const float* kernel2 = kernel_data + (p + 2) * inch + q;
				const float* kernel3 = kernel_data + (p + 3) * inch + q;
				const float* kernel4 = kernel_data + (p + 4) * inch + q;
				const float* kernel5 = kernel_data + (p + 5) * inch + q;
				const float* kernel6 = kernel_data + (p + 6) * inch + q;
				const float* kernel7 = kernel_data + (p + 7) * inch + q;
