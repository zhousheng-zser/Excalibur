...
namespace glasssix
{
	namespace excalibur
	{
		...
		private:
			std::vector<std::shared_ptr<memory::tensor<float>>> weights_transformed_vec_;
			std::shared_ptr<memory::tensor<float>> weights_transformed_;
			std::shared_ptr<memory::tensor<float>> weights_sgemm_;
