...
namespace glasssix
{
	namespace excalibur
	{
		...
		public:
			...
			conv_depthwise_native_gpu(int input_Channel, int output_Channel, int kernelSize, int group, int stride, int pad, bool bias_term, int device) 
				: baseconv(input_Channel, output_Channel, kernelSize, group, stride, pad, bias_term, device) {}

			virtual ~conv_depthwise_native_gpu() {}
