#ifndef _BLUR_VSL_NET_HPP_
#define _BLUR_VSL_NET_HPP_
...
namespace glasssix
{
	namespace longinus
	{
		class Blur_vsl_net:public Selene
		{
			...
			Neuron_Name(conv6_1);
			Declear_Opration(softmax, cls_loss);
		...
		public:
			...
			virtual ~Blur_vsl_net();

			void Forward(const float* input_data, int order = 0)
			{
				if (order == 0)//NCHW
				{
				...
				else
				{
					...
					CUDA_CHECK(cudaMemcpy(tensor_data, input_data, 1 * 3 * 48 * 48 * sizeof(float), cudaMemcpyDefault));
			...
			void Forward(const unsigned char* input_data, int order = 0)
			{
				if (order == 0)//NCHW
				{
				...
				if (device_ < 0)
				{
					...
					memcpy(tensor_data, input_data, 1 * 3 * 48 * 48 * sizeof(unsigned char));
				...
				else
				{
					...
					CUDA_CHECK(cudaMemcpy(tensor_data, input_data, 1 * 3 * 48 * 48 * sizeof(unsigned char), cudaMemcpyDefault));
