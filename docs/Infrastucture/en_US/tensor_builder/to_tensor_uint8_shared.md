...
A smart pointer known as **```std::shared_ptr<glasssix::tensor<uint8_t>>```**, which contains the converted tensor if the operation was successful and otherwise, an null pointer is included.
