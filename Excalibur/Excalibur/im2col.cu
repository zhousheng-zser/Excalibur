		...
		__global__ void im2col_gpu_kernel(const int num_of_threads, const Dtype* data_im,
			...
			if (order == NCHW)
			{
				CUDA_KERNEL_LOOP(index, num_of_threads) {
					...
					data_col_ptr += (n * channels * kernel_h *kernel_w + c_col * height_col + h_col) * width_col + w_col;
			...
			else if (order == NHWC)
			{
				CUDA_KERNEL_LOOP(index, num_of_threads) {
					const int n = index / (channels * height_col * width_col);
					size_t remain = index % (channels * height_col * width_col);
					const int c_im = remain % channels;
					remain = remain / channels;
					const int h_col = remain / width_col;
					const int w_col = remain % width_col;
					const int c_col = c_im * kernel_h * kernel_w;
					const int h_offset = h_col * stride_h - pad_h;
					const int w_offset = w_col * stride_w - pad_w;
					...
					data_col_ptr += (n * channels * kernel_h *kernel_w + c_col * height_col + h_col) * width_col + w_col;
					const Dtype* data_im_ptr = data_im;
					data_im_ptr += ((n * height * channels + h_offset) * width + w_offset) * channels + c_im;
					for (int i = 0; i < kernel_h; ++i) {
						for (int j = 0; j < kernel_w; ++j) {
							int h_im = h_offset + i * dilation_h;
							int w_im = w_offset + j * dilation_w;
