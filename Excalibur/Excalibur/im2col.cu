...
namespace glasssix
{
	namespace excalibur
	{
		...
		__global__ void im2col_gpu_kernel(const int num_of_threads, const Dtype* data_im,
			...
			if (order == NCHW)
			{
				CUDA_KERNEL_LOOP(index, num_of_threads) {
					const int n = index / (channels * height_col * width_col);
					size_t remain = index % (channels * height_col * width_col);
					const int h_index = remain / width_col;
					const int h_col = h_index % height_col;
					const int w_col = remain % width_col;
					const int c_im = h_index / height_col;
					const int c_col = c_im * kernel_h * kernel_w;
					const int h_offset = h_col * stride_h - pad_h;
					const int w_offset = w_col * stride_w - pad_w;
					Dtype* data_col_ptr = data_col;
					data_col_ptr += (n * channels * kernel_h * kernel_w * height_col + c_col * height_col + h_col) * width_col + w_col;
					const Dtype* data_im_ptr = data_im;
					data_im_ptr += (n * height * channels + c_im * height + h_offset) * width + w_offset;
			...
			else if (order == NHWC)
			{
				CUDA_KERNEL_LOOP(index, num_of_threads) {
					...
					data_col_ptr += (n * channels * kernel_h * kernel_w * height_col + c_col * height_col + h_col) * width_col + w_col;
					const Dtype* data_im_ptr = data_im;
					data_im_ptr += (n * height * width * channels + (h_offset * width + w_offset) * channels) + c_im;
					for (int i = 0; i < kernel_h; ++i) {
						for (int j = 0; j < kernel_w; ++j) {
							int h_im = h_offset + i * dilation_h;
							int w_im = w_offset + j * dilation_w;
							*data_col_ptr =
								(h_im >= 0 && w_im >= 0 && h_im < height && w_im < width) ?
								data_im_ptr[(i * dilation_h * width + j * dilation_w) * channels] : 0;
							data_col_ptr += height_col * width_col;
						}
					}
				}
		...
		void im2col_gpu(const Dtype* data_im, const int channels,
			...
			im2col_gpu_kernel<Dtype> << <CUDA_GET_BLOCKS(num_kernels),
				CUDA_NUM_THREADS >> >(
					num_kernels, data_im, channels, height, width, kernel_h, kernel_w, pad_h,
					pad_w, stride_h, stride_w, dilation_h, dilation_w, height_col,
					width_col, data_col, order, num);
		...
		template void im2col_gpu<float>(const float* data_im, const int channels,
			...
			const int dilation_h, const int dilation_w, float* data_col, orderType order = NCHW, int num = 1);

		template void im2col_gpu<unsigned char>(const unsigned char* data_im, const int channels,
			const int height, const int width, const int kernel_h, const int kernel_w,
			const int pad_h, const int pad_w, const int stride_h, const int stride_w,
			const int dilation_h, const int dilation_w, unsigned char* data_col, orderType order = NCHW, int num = 1);
		...
		void im2col_nd_gpu(const Dtype* data_im, const int num_spatial_axes,
			...
			CHECK_LT(num_spatial_axes, CUDA_NUM_THREADS);
			switch (num_spatial_axes) {
			case 1:
				im2col_nd_gpu_kernel<Dtype, 1>  // NOLINT_NEXT_LINE(whitespace/operators)
					<< <CUDA_GET_BLOCKS(num_kernels), CUDA_NUM_THREADS >> >(
		...
		void col2im_nd_gpu(const Dtype* data_col, const int num_spatial_axes,
			...
			case 1:
				col2im_nd_gpu_kernel<Dtype, 1>  // NOLINT_NEXT_LINE(whitespace/operators)
					<< <CUDA_GET_BLOCKS(im_size), CUDA_NUM_THREADS >> >(
