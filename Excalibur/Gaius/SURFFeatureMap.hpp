#pragma once
#ifndef _SURFFEATUREMAP_HPP_
#define _SURFFEATUREMAP_HPP_
...
namespace excalibur
{
	...
	public:
		...
		std::vector<SURFFeature>::const_iterator begin() const {
			return pool_.begin();
		}

		std::vector<SURFFeature>::const_iterator end() const {
			return pool_.end();
		}

		const SURFFeature & operator[](std::size_t idx) const {
			return pool_[idx];
		}
	...
	public:
		...
		virtual void SetROI(const Rect & roi) {
			roi_ = roi;
			if (buf_valid_reset_) {
		...
		int32_t GetFeatureVectorDim(int32_t feat_id) const {
