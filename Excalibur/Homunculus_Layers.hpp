...
namespace Excalibur
{
	...
	*/
	...
	public:
		...
		* layer.
		*/
		explicit Homunculus_Layers(const caffe::LayerParameter& param)
			: layer_param_(param) {
			// Set phase and copy blobs (if there are any).
			if (layer_param_.blobs_size() > 0) {
				blobs_.resize(layer_param_.blobs_size());
				for (int i = 0; i < layer_param_.blobs_size(); ++i) {
					blobs_[i].reset(new Pandora_Blob<Dtype>);
		...
		*/
		void SetUp(const std::vector<Pandora_Blob<Dtype>*>& bottom, const std::vector<Pandora_Blob<Dtype>*>& top) {
		...
		*/
		virtual void LayerSetUp(const std::vector<Pandora_Blob<Dtype>*>& bottom,
		...
		*/
		inline void Forward(const std::vector<Pandora_Blob<Dtype>*>& bottom,
			const std::vector<Pandora_Blob<Dtype>*>& top);
		...
		*/
		std::vector<std::shared_ptr<Pandora_Blob<Dtype>> >& blobs() {
			return blobs_;
		}
		...
		*/
		virtual void ToProto(caffe::LayerParameter* param);
	...
	protected:
		...
		*/
		virtual void CheckBlobCounts(const std::vector<Pandora_Blob<Dtype>*>& bottom,
			const std::vector<Pandora_Blob<Dtype>*>& top) {
