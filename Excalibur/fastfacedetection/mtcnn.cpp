...
#pragma omp parallel for num_threads(threads_num)
		...
		std::vector<FaceInfoX> MTCNN::ProposalNet(const unsigned char* image, const int channels, const int height, const int width, 
		...
		std::vector<FaceInfoX> MTCNN::NextStage(const unsigned char* image, const int channels, const int height, const int width, 
			std::vector<FaceInfoX> &pre_stage_res, int input_w, int input_h, int stage_num, const float threshold)
		{
...
#pragma omp parallel for num_threads(threads_num)
		...
		std::vector<FaceInfoX> MTCNN::Detect(const unsigned char* image, const int channels, const int height, const int width, 
			const int minSize, const float* threshold, const float factor, const int stage)
		{
			...
			if (stage >= 2 && pnet_res.size()>0) {
				...
				for (int iter = 0; iter < size; ++iter) {
					...
					std::vector<FaceInfoX> res = NextStage(image, channels, height, width, input, 24, 24, 2, threshold[1]);
			...
			if (stage >= 3 && rnet_res.size()>0) {
				...
				for (int iter = 0; iter < size; ++iter) {
					...
					std::vector<FaceInfoX> res = NextStage(image, channels, height, width, input, 48, 48, 3, threshold[2]);
