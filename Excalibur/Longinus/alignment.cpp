...
namespace glasssix
{
	...
	rectangle<int> alignment::calc_margin_rect(rectangle<int>& face_rect, point<float>& center, float roll, float pitch)
	{
		int delta = (int)(sin(roll) * face_rect.h / 2);
		face_rect.x += delta;
		center.x += delta;
		int delta_pitch = (int)((1 - cos((double)pitch * 90 * 3.141592653 / 180))*face_rect.h / 2);

		face_rect.y -= delta_pitch;
		face_rect.h += 2 * delta_pitch;

		int margin_height = (int)(face_rect.h * 1.2);
		int margin_x = (int)(center.x - margin_height / 2);
		int margin_y = (int)(center.y - margin_height / 2);
		return rectangle<int>(margin_x, margin_y, margin_height, margin_height);
	}
