...
namespace Excalibur {
	...
	bool UpgradeNetAsNeeded(const std::string& param_file, caffe::NetParameter* param) {
		...
		// caffe::NetParameter uses old style input fields; try to upgrade it.
	...
	void ReadNetParamsFromTextFileOrDie(const std::string& param_file,
		...
		CHECK(ReadProtoFromTextFile(param_file, param))
			<< "Failed to parse caffe::NetParameter file: " << param_file;
	...
	bool UpgradeV1Net(const caffe::NetParameter& v1_net_param, caffe::NetParameter* net_param) {
		if (v1_net_param.layer_size() > 0) {
			LOG(FATAL) << "Refusing to upgrade inconsistent caffe::NetParameter input; "
				<< "the definition includes both 'layer' and 'layers' fields. "
				<< "The current format defines 'layer' fields with std::string type like "
	...
	const char* UpgradeV1LayerType(const caffe::V1LayerParameter_LayerType type) {
		...
		case caffe::V1LayerParameter_LayerType_THRESHOLD:
			return "Threshold";
		default:
			LOG(FATAL) << "Unknown caffe::V1LayerParameter layer type: " << type;
			return "";
		}
	}
	...
	void UpgradeNetInput(caffe::NetParameter* net_param) {
		...
		// If the caffe::NetParameter holds an input alone, without shape/dim, then
