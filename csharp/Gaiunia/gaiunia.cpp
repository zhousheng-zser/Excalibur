...
namespace glasssix
{
	namespace gaius
	{
		Gaiunia::Gaiunia(int device)
		{
			this->device_ = device;
			gaius_wrapper = new GaiusFeature(device_);
		}
		...
		Gaiunia::!Gaiunia()
		{
			delete gaius_wrapper;
			gaius_wrapper = nullptr;
		}

		unsigned char* Gaiunia::Bitmaps2RGBs(array<Bitmap^>^ bmps)
		{
			int num = bmps->Length;
			if (num <= 0)
			{
		...
		array<float>^ Gaiunia::ExtractBitmapOutputs(array<Bitmap^>^ imgDatas)
		{
