...
namespace glasssix
{
	namespace excalibur
	{
		...
		void operation_convolution<Dtype>::conv3x3s1_winograd23_tr_kernel()
		{
			...
			int inch = input_channel_ / group_;
			int outch = output_channel_ / group_;

			const float* kernel_data_base = weights_f32_[0]->cpu_data();
			// G
			const float ktm[4][3] = {
				{ 1.0f,     0.0f,     0.0f },
			...
			{
				kernel_tm_.push_back(std::shared_ptr<memory::tensor<float>>(new memory::tensor<float>(std::vector<int>{ 1, outch, inch, 4 * 4}, params_.device_, memory::NCHW, nullptr)));
				...
				const float* kernel_data = kernel_data_base + weight_data_size_ * g / group_;
...
#pragma omp parallel for num_threads(2) 
			...
			std::shared_ptr<memory::tensor<float> >& top_blob)
		{
			...
			int inch = bottom_blob->channels() / group_;
			...
			int ou = top_blob->channels() / group_;
			...
			if (bias_term_)
			{
				bias_0 = weights_f32_[1]->cpu_data();
			}

			top_blob_bordered_.reset(new memory::tensor<float>(std::vector<int>{num, output_channel_, outh, outw}, -1, memory::NCHW, bottom_blob->allocator()));
			float* top_data_base = top_blob_bordered_->mutable_cpu_data();

			for (int n = 0; n < num; n++)
			{
				float* bottom_data_base = bottom_blob_bordered_data + n * inch * bordered_h * bordered_w * group_;
				for (int g = 0; g < group_; g++)
				{
...
#pragma omp parallel for num_threads(2) 
		...
		void operation_convolution<Dtype>::forward_im2col_tr_kernel()
		{
			...
			const float* kernel_base = weights_f32_[0]->cpu_data();
			...
			for (int g = 0; g < group_; g++)
			{
				kernel_tm_gemm_.push_back(std::shared_ptr<memory::tensor<float>>(new memory::tensor<float>(std::vector<int>{ 1, kernel_tm_channel, inch, 8 * kernel_size}, params_.device_, memory::NCHW, nullptr)));
				float* kernel_tm = kernel_tm_gemm_[g]->mutable_cpu_data();
				int kernel_tm_cstep = kernel_tm_gemm_[g]->width() * kernel_tm_gemm_[g]->height();
				const float* kernel = kernel_base + weight_data_size_ * g / group_;
#ifdef _OPENMP 
#pragma omp parallel for num_threads(2) 
#endif
			...
			std::shared_ptr<memory::tensor<float> >& top_blob)
		{
			...
			int outw = top_blob->width();
			int outh = top_blob->height();
			int outch = top_blob->channels() / group_;
