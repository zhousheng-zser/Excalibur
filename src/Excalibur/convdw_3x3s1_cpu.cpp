...
namespace glasssix
{
	namespace excalibur
	{
		convdw_3x3s1_cpu::convdw_3x3s1_cpu(int input_Channel, int output_Channel, int group, int kernelSize, int stride, int pad, bool bias_term, int device, bool int8_quantization)
			: baseconv(input_Channel, output_Channel, group, kernelSize, stride, pad, bias_term, device, int8_quantization)
		{

		}

		void convdw_3x3s1_cpu::forward_gemm(const signed char* input, const signed char* weights, int* output, bool skip_im2col) {}

		void convdw_3x3s1_cpu::forward_gemm(const float* input, const float* weights, float* output, bool skip_im2col) {}

		void convdw_3x3s1_cpu::forward_bias(float* output, const float* bias_data) {}

		void convdw_3x3s1_cpu::Forward(const std::shared_ptr<tensor<float>>& bottom, std::shared_ptr<tensor<float>>& top)
		{
