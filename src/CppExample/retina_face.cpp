...
namespace glasssix
{
	...
	retina_face::retina_face(std::string phai_path, std::string racy_path, float nms, int device)
		: nms_threshold(nms), device_(device)
	{
		ratio_ = { 1.0 };
		//anchor setting
	...
	std::vector<face_detect_info> retina_face::detect(cv::Mat img, float threshold, float scales)
	{
		...
		std::vector<face_detect_info> face_info;
		for (size_t i = 0; i < feat_stride_fpn_.size(); i++)
		{
			...
			for (size_t num = 0; num < num_anchor; num++)
			{
				for (size_t j = 0; j < count; j++)
				{
					...
					tmp.rect = rect;
					tmp.pts = landmarks;
					face_info.push_back(tmp);
				}
			}
		}

		face_info = nms(face_info, nms_threshold);
