...
#pragma omp parallel for 
			...
			inline void adddot1x1(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re = _mm256_setzero_si256();
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					const __m128i a = _mm_load_si128((__m128i*)(packedA_ptr + j));
					const __m128i b = _mm_load_si128((__m128i*)(packedB_ptr + j));
					re = _mm256_madd_epi16_epi32(a, b, re);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re) + beta * *C_ptr;
			}

			//adddot1x1(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot1x1(padK, alpha,  packedA_ptr, packedB_ptr + padK, beta, C_ptr + 1, ldc);
			inline void adddot1x2(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re1 = _mm256_setzero_si256();
				__m256i re2 = _mm256_setzero_si256();
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					const __m128i a = _mm_load_si128((__m128i*)(packedA_ptr + j));
					const __m128i b1 = _mm_load_si128((__m128i*)(packedB_ptr + j));
					const __m128i b2 = _mm_load_si128((__m128i*)(packedB_ptr + padK + j));
					re1 = _mm256_madd_epi16_epi32(a, b1, re1);
					re2 = _mm256_madd_epi16_epi32(a, b2, re2);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re1) + beta * *C_ptr;
				*(C_ptr + 1) = alpha * _mm256_sumall_epi32(re2) + beta * *(C_ptr + 1);
			}

			//adddot1x1(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot1x1(padK, alpha,  packedA_ptr + padK, packedB_ptr, beta, C_ptr + ldc, ldc);
			inline void adddot2x1(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re1 = _mm256_setzero_si256();
				__m256i re2 = _mm256_setzero_si256();
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					const __m128i a1 = _mm_load_si128((__m128i*)(packedA_ptr + j));
					const __m128i a2 = _mm_load_si128((__m128i*)(packedA_ptr + padK + j));
					const __m128i b = _mm_load_si128((__m128i*)(packedB_ptr + j));
					re1 = _mm256_madd_epi16_epi32(a1, b, re1);
					re2 = _mm256_madd_epi16_epi32(a2, b, re2);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re1) + beta * *C_ptr;
				*(C_ptr + 1) = alpha * _mm256_sumall_epi32(re2) + beta * *(C_ptr + ldc);
			}

			//adddot1x2(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot1x2(padK, alpha,  packedA_ptr, packedB_ptr + padK * 2, beta, C_ptr + 2, ldc);
			inline void adddot1x4(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[4] = { _mm256_setzero_si256() };
				__m128i a;
				__m128i b[4];
				const int padK_offset[4] = { padK * 0, padK * 1, padK * 2, padK * 3 };
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					a = _mm_load_si128((__m128i*)(packedA_ptr + j));
					b[0] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[0] + j));
					b[1] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[1] + j));
					b[2] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[2] + j));
					b[3] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[3] + j));
					re[0] = _mm256_madd_epi16_epi32(a, b[0], re[0]);
					re[1] = _mm256_madd_epi16_epi32(a, b[1], re[1]);
					re[2] = _mm256_madd_epi16_epi32(a, b[2], re[2]);
					re[3] = _mm256_madd_epi16_epi32(a, b[3], re[3]);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re[0]) + beta * *C_ptr;
				*(C_ptr + 1) = alpha * _mm256_sumall_epi32(re[1]) + beta * *(C_ptr + 1);
				*(C_ptr + 2) = alpha * _mm256_sumall_epi32(re[2]) + beta * *(C_ptr + 2);
				*(C_ptr + 3) = alpha * _mm256_sumall_epi32(re[3]) + beta * *(C_ptr + 3);
			}

			//adddot1x2(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot1x2(padK, alpha,  packedA_ptr + padK, packedB_ptr, beta, C_ptr + ldc, ldc);
			inline void adddot2x2(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[4] = { _mm256_setzero_si256() };
				__m128i a[2];
				...
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					a[0] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[0] + j));
					a[1] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[1] + j));
					b[0] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[0] + j));
					b[1] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[1] + j));
					re[0] = _mm256_madd_epi16_epi32(a[0], b[0], re[0]);
					re[1] = _mm256_madd_epi16_epi32(a[0], b[1], re[1]);
					re[2] = _mm256_madd_epi16_epi32(a[1], b[0], re[2]);
					re[3] = _mm256_madd_epi16_epi32(a[1], b[1], re[3]);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re[0]) + beta * *C_ptr;
				*(C_ptr + 1) = alpha * _mm256_sumall_epi32(re[1]) + beta * *(C_ptr + 1);
				*(C_ptr + ldc) = alpha * _mm256_sumall_epi32(re[2]) + beta * *(C_ptr + ldc);
				*(C_ptr + ldc + 1) = alpha * _mm256_sumall_epi32(re[3]) + beta * *(C_ptr + ldc + 1);
			}

			//adddot2x1(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot2x1(padK, alpha,  packedA_ptr + padK * 2, packedB_ptr, beta, C_ptr + 2 * ldc, ldc);
			inline void adddot4x1(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[4] = { _mm256_setzero_si256() };
				__m128i a[4];
				...
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					b = _mm_load_si128((__m128i*)(packedB_ptr + j));
					a[0] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[0] + j));
					a[1] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[1] + j));
					a[2] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[2] + j));
					a[3] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[3] + j));
					re[0] = _mm256_madd_epi16_epi32(a[0], b, re[0]);
					re[1] = _mm256_madd_epi16_epi32(a[1], b, re[1]);
					re[2] = _mm256_madd_epi16_epi32(a[2], b, re[2]);
					re[3] = _mm256_madd_epi16_epi32(a[3], b, re[3]);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re[0]) + beta * *C_ptr;
				*(C_ptr + 1 * ldc) = alpha * _mm256_sumall_epi32(re[1]) + beta * *(C_ptr + 1 * ldc);
				*(C_ptr + 2 * ldc) = alpha * _mm256_sumall_epi32(re[2]) + beta * *(C_ptr + 2 * ldc);
				*(C_ptr + 3 * ldc) = alpha * _mm256_sumall_epi32(re[3]) + beta * *(C_ptr + 3 * ldc);
			}

			//adddot1x4(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot1x4(padK, alpha,  packedA_ptr, packedB_ptr + padK * 4, beta, C_ptr + 4, ldc);*/
			inline void adddot1x8(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[8] = { _mm256_setzero_si256() };
				__m128i a;
				__m128i b[8];
				const int padK_offset[8] = { padK * 0, padK * 1, padK * 2, padK * 3, padK * 4, padK * 5, padK * 6, padK * 7 };
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					...
					b[2] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[2] + j));
					b[3] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[3] + j));
					b[4] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[4] + j));
					b[5] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[5] + j));
					b[6] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[6] + j));
					b[7] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[7] + j));
					re[0] = _mm256_madd_epi16_epi32(a, b[0], re[0]);
					re[1] = _mm256_madd_epi16_epi32(a, b[1], re[1]);
					re[2] = _mm256_madd_epi16_epi32(a, b[2], re[2]);
					re[3] = _mm256_madd_epi16_epi32(a, b[3], re[3]);
					re[4] = _mm256_madd_epi16_epi32(a, b[4], re[4]);
					re[5] = _mm256_madd_epi16_epi32(a, b[5], re[5]);
					re[6] = _mm256_madd_epi16_epi32(a, b[6], re[6]);
					re[7] = _mm256_madd_epi16_epi32(a, b[7], re[7]);
				}
				for (int i = 0; i < 8; i++)
				{
					*(C_ptr + i) = alpha * _mm256_sumall_epi32(re[i]) + beta * *(C_ptr + i);
				}
			}

			//adddot2x2(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot2x2(padK, alpha,  packedA_ptr, packedB_ptr + padK * 2, beta, C_ptr + 2, ldc);
			inline void adddot2x4(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[8] = { _mm256_setzero_si256() };
				__m128i a[2];
				...
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					...
					re[0] = _mm256_madd_epi16_epi32(a[0], b[0], re[0]);
					re[1] = _mm256_madd_epi16_epi32(a[0], b[1], re[1]);
					re[2] = _mm256_madd_epi16_epi32(a[0], b[2], re[2]);
					re[3] = _mm256_madd_epi16_epi32(a[0], b[3], re[3]);
					re[4] = _mm256_madd_epi16_epi32(a[1], b[0], re[4]);
					re[5] = _mm256_madd_epi16_epi32(a[1], b[1], re[5]);
					re[6] = _mm256_madd_epi16_epi32(a[1], b[2], re[6]);
					re[7] = _mm256_madd_epi16_epi32(a[1], b[3], re[7]);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re[0]) + beta * *C_ptr;
				*(C_ptr + 1) = alpha * _mm256_sumall_epi32(re[1]) + beta * *(C_ptr + 1);
				*(C_ptr + 2) = alpha * _mm256_sumall_epi32(re[2]) + beta * *(C_ptr + 2);
				*(C_ptr + 3) = alpha * _mm256_sumall_epi32(re[3]) + beta * *(C_ptr + 3);
				*(C_ptr + ldc) = alpha * _mm256_sumall_epi32(re[4]) + beta * *(C_ptr + ldc);
				*(C_ptr + ldc + 1) = alpha * _mm256_sumall_epi32(re[5]) + beta * *(C_ptr + ldc + 1);
				*(C_ptr + ldc + 2) = alpha * _mm256_sumall_epi32(re[6]) + beta * *(C_ptr + ldc + 2);
				*(C_ptr + ldc + 3) = alpha * _mm256_sumall_epi32(re[7]) + beta * *(C_ptr + ldc + 3);
			}

			//adddot2x2(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot2x2(padK, alpha,  packedA_ptr + padK * 2, packedB_ptr, beta, C_ptr + 2 * ldc, ldc);
			inline void adddot4x2(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[8] = { _mm256_setzero_si256() };
				__m128i a[4];
				...
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					...
					b[1] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[1] + j));
					re[0] = _mm256_madd_epi16_epi32(a[0], b[0], re[0]);
					re[1] = _mm256_madd_epi16_epi32(a[1], b[0], re[1]);
					re[2] = _mm256_madd_epi16_epi32(a[2], b[0], re[2]);
					re[3] = _mm256_madd_epi16_epi32(a[3], b[0], re[3]);
					re[4] = _mm256_madd_epi16_epi32(a[0], b[1], re[4]);
					re[5] = _mm256_madd_epi16_epi32(a[1], b[1], re[5]);
					re[6] = _mm256_madd_epi16_epi32(a[2], b[1], re[6]);
					re[7] = _mm256_madd_epi16_epi32(a[3], b[1], re[7]);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re[0]) + beta * *C_ptr;
				*(C_ptr + ldc * 1) = alpha * _mm256_sumall_epi32(re[1]) + beta * *(C_ptr + ldc * 1);
				*(C_ptr + ldc * 2) = alpha * _mm256_sumall_epi32(re[2]) + beta * *(C_ptr + ldc * 2);
				*(C_ptr + ldc * 3) = alpha * _mm256_sumall_epi32(re[3]) + beta * *(C_ptr + ldc * 3);
				*(C_ptr + 1) = alpha * _mm256_sumall_epi32(re[4]) + beta * *(C_ptr + 1);
				*(C_ptr + ldc * 1 + 1) = alpha * _mm256_sumall_epi32(re[5]) + beta * *(C_ptr + ldc * 1 + 1);
				*(C_ptr + ldc * 2 + 1) = alpha * _mm256_sumall_epi32(re[6]) + beta * *(C_ptr + ldc * 2 + 1);
				*(C_ptr + ldc * 3 + 1) = alpha * _mm256_sumall_epi32(re[7]) + beta * *(C_ptr + ldc * 3 + 1);
			}

			//adddot4x1(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot4x1(padK, alpha,  packedA_ptr + padK * 4, packedB_ptr, beta, C_ptr + 4 * ldc, ldc);
			inline void adddot8x1(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[8] = { _mm256_setzero_si256() };
				__m128i a[8];
				...
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					...
					a[2] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[2] + j));
					a[3] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[3] + j));
					a[4] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[4] + j));
					a[5] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[5] + j));
					a[6] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[6] + j));
					a[7] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[7] + j));
					b = _mm_load_si128((__m128i*)(packedB_ptr + j));
					re[0] = _mm256_madd_epi16_epi32(a[0], b, re[0]);
					re[1] = _mm256_madd_epi16_epi32(a[1], b, re[1]);
					re[2] = _mm256_madd_epi16_epi32(a[2], b, re[2]);
					re[3] = _mm256_madd_epi16_epi32(a[3], b, re[3]);
					re[4] = _mm256_madd_epi16_epi32(a[4], b, re[4]);
					re[5] = _mm256_madd_epi16_epi32(a[5], b, re[5]);
					re[6] = _mm256_madd_epi16_epi32(a[6], b, re[6]);
					re[7] = _mm256_madd_epi16_epi32(a[7], b, re[7]);
				}
				for (int i = 0; i < 8; i++)
				{
					*(C_ptr + ldc * i) = alpha * _mm256_sumall_epi32(re[i]) + beta * *(C_ptr + ldc * i);
				}
			}

			//adddot1x8(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot1x8(padK, alpha,  packedA_ptr, packedB_ptr + padK * 8, beta, C_ptr + 8, ldc);
			inline void adddot1x16(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[16] = { _mm256_setzero_si256() };
				__m128i a;
				__m128i b[16];
				const int padK_offset[16] = { padK * 0, padK * 1, padK * 2, padK * 3, padK * 4, padK * 5, padK * 6, padK * 7,
				padK * 8, padK * 9, padK * 10, padK * 11, padK * 12, padK * 13, padK * 14, padK * 15 };
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					...
					b[6] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[6] + j));
					b[7] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[7] + j));
					b[8] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[8] + j));
					b[9] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[9] + j));
					b[10] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[10] + j));
					b[11] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[11] + j));
					b[12] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[12] + j));
					b[13] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[13] + j));
					b[14] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[14] + j));
					b[15] = _mm_load_si128((__m128i*)(packedB_ptr + padK_offset[15] + j));
					re[0] = _mm256_madd_epi16_epi32(a, b[0], re[0]);
					re[1] = _mm256_madd_epi16_epi32(a, b[1], re[1]);
					re[2] = _mm256_madd_epi16_epi32(a, b[2], re[2]);
					re[3] = _mm256_madd_epi16_epi32(a, b[3], re[3]);
					...
					re[6] = _mm256_madd_epi16_epi32(a, b[6], re[6]);
					re[7] = _mm256_madd_epi16_epi32(a, b[7], re[7]);
					re[8] = _mm256_madd_epi16_epi32(a, b[8], re[8]);
					re[9] = _mm256_madd_epi16_epi32(a, b[9], re[9]);
					re[10] = _mm256_madd_epi16_epi32(a, b[10], re[10]);
					re[11] = _mm256_madd_epi16_epi32(a, b[11], re[11]);
					re[12] = _mm256_madd_epi16_epi32(a, b[12], re[12]);
					re[13] = _mm256_madd_epi16_epi32(a, b[13], re[13]);
					re[14] = _mm256_madd_epi16_epi32(a, b[14], re[14]);
					re[15] = _mm256_madd_epi16_epi32(a, b[15], re[15]);
				}
				for (int i = 0; i < 16; i++)
				{
					*(C_ptr + i) = alpha * _mm256_sumall_epi32(re[i]) + beta * *(C_ptr + i);
			...
			//adddot2x4(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot2x4(padK, alpha,  packedA_ptr, packedB_ptr + padK * 4, beta, C_ptr + 4, ldc);
			inline void adddot2x8(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[16] = { _mm256_setzero_si256() };
				__m128i a[2];
				...
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					...
					re[2] = _mm256_madd_epi16_epi32(a[0], b[2], re[2]);
					re[3] = _mm256_madd_epi16_epi32(a[0], b[3], re[3]);
					re[4] = _mm256_madd_epi16_epi32(a[0], b[4], re[4]);
					re[5] = _mm256_madd_epi16_epi32(a[0], b[5], re[5]);
					re[6] = _mm256_madd_epi16_epi32(a[0], b[6], re[6]);
					re[7] = _mm256_madd_epi16_epi32(a[0], b[7], re[7]);
					re[8] = _mm256_madd_epi16_epi32(a[1], b[0], re[8]);
					re[9] = _mm256_madd_epi16_epi32(a[1], b[1], re[9]);
					re[10] = _mm256_madd_epi16_epi32(a[1], b[2], re[10]);
					re[11] = _mm256_madd_epi16_epi32(a[1], b[3], re[11]);
					re[12] = _mm256_madd_epi16_epi32(a[1], b[4], re[12]);
					re[13] = _mm256_madd_epi16_epi32(a[1], b[5], re[13]);
					re[14] = _mm256_madd_epi16_epi32(a[1], b[6], re[14]);
					re[15] = _mm256_madd_epi16_epi32(a[1], b[7], re[15]);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re[0]) + beta * *C_ptr;
				*(C_ptr + 1) = alpha * _mm256_sumall_epi32(re[1]) + beta * *(C_ptr + 1);
				*(C_ptr + 2) = alpha * _mm256_sumall_epi32(re[2]) + beta * *(C_ptr + 2);
				*(C_ptr + 3) = alpha * _mm256_sumall_epi32(re[3]) + beta * *(C_ptr + 3);
				*(C_ptr + 4) = alpha * _mm256_sumall_epi32(re[4]) + beta * *(C_ptr + 4);
				*(C_ptr + 5) = alpha * _mm256_sumall_epi32(re[5]) + beta * *(C_ptr + 5);
				*(C_ptr + 6) = alpha * _mm256_sumall_epi32(re[6]) + beta * *(C_ptr + 6);
				*(C_ptr + 7) = alpha * _mm256_sumall_epi32(re[7]) + beta * *(C_ptr + 7);
				*(C_ptr + ldc) = alpha * _mm256_sumall_epi32(re[8]) + beta * *(C_ptr + ldc);
				*(C_ptr + ldc + 1) = alpha * _mm256_sumall_epi32(re[9]) + beta * *(C_ptr + ldc + 1);
				*(C_ptr + ldc + 2) = alpha * _mm256_sumall_epi32(re[10]) + beta * *(C_ptr + ldc + 2);
				*(C_ptr + ldc + 3) = alpha * _mm256_sumall_epi32(re[11]) + beta * *(C_ptr + ldc + 3);
				*(C_ptr + ldc + 4) = alpha * _mm256_sumall_epi32(re[12]) + beta * *(C_ptr + ldc + 4);
				*(C_ptr + ldc + 5) = alpha * _mm256_sumall_epi32(re[13]) + beta * *(C_ptr + ldc + 5);
				*(C_ptr + ldc + 6) = alpha * _mm256_sumall_epi32(re[14]) + beta * *(C_ptr + ldc + 6);
				*(C_ptr + ldc + 7) = alpha * _mm256_sumall_epi32(re[15]) + beta * *(C_ptr + ldc + 7);
			}

			//adddot2x4(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot2x4(padK, alpha,  packedA_ptr + 2 * padK, packedB_ptr, beta, C_ptr + 2 * ldc, ldc);
			inline void adddot4x4(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[16] = { _mm256_setzero_si256() };
				__m128i a[4];
				...
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					...
					re[6] = _mm256_madd_epi16_epi32(a[1], b[2], re[6]);
					re[7] = _mm256_madd_epi16_epi32(a[1], b[3], re[7]);
					re[8] = _mm256_madd_epi16_epi32(a[2], b[0], re[8]);
					re[9] = _mm256_madd_epi16_epi32(a[2], b[1], re[9]);
					re[10] = _mm256_madd_epi16_epi32(a[2], b[2], re[10]);
					re[11] = _mm256_madd_epi16_epi32(a[2], b[3], re[11]);
					re[12] = _mm256_madd_epi16_epi32(a[3], b[0], re[12]);
					re[13] = _mm256_madd_epi16_epi32(a[3], b[1], re[13]);
					re[14] = _mm256_madd_epi16_epi32(a[3], b[2], re[14]);
					re[15] = _mm256_madd_epi16_epi32(a[3], b[3], re[15]);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re[0]) + beta * *C_ptr;
				*(C_ptr + 1) = alpha * _mm256_sumall_epi32(re[1]) + beta * *(C_ptr + 1);
				*(C_ptr + 2) = alpha * _mm256_sumall_epi32(re[2]) + beta * *(C_ptr + 2);
				*(C_ptr + 3) = alpha * _mm256_sumall_epi32(re[3]) + beta * *(C_ptr + 3);
				*(C_ptr + ldc * 1) = alpha * _mm256_sumall_epi32(re[4]) + beta * *(C_ptr + ldc * 1);
				*(C_ptr + ldc * 1 + 1) = alpha * _mm256_sumall_epi32(re[5]) + beta * *(C_ptr + ldc * 1 + 1);
				*(C_ptr + ldc * 1 + 2) = alpha * _mm256_sumall_epi32(re[6]) + beta * *(C_ptr + ldc * 1 + 2);
				*(C_ptr + ldc * 1 + 3) = alpha * _mm256_sumall_epi32(re[7]) + beta * *(C_ptr + ldc * 1 + 3);
				*(C_ptr + ldc * 2) = alpha * _mm256_sumall_epi32(re[8]) + beta * *(C_ptr + ldc * 2);
				*(C_ptr + ldc * 2 + 1) = alpha * _mm256_sumall_epi32(re[9]) + beta * *(C_ptr + ldc * 2 + 1);
				*(C_ptr + ldc * 2 + 2) = alpha * _mm256_sumall_epi32(re[10]) + beta * *(C_ptr + ldc * 2 + 2);
				*(C_ptr + ldc * 2 + 3) = alpha * _mm256_sumall_epi32(re[11]) + beta * *(C_ptr + ldc * 2 + 3);
				*(C_ptr + ldc * 3) = alpha * _mm256_sumall_epi32(re[12]) + beta * *(C_ptr + ldc * 3);
				*(C_ptr + ldc * 3 + 1) = alpha * _mm256_sumall_epi32(re[13]) + beta * *(C_ptr + ldc * 3 + 1);
				*(C_ptr + ldc * 3 + 2) = alpha * _mm256_sumall_epi32(re[14]) + beta * *(C_ptr + ldc * 3 + 2);
				*(C_ptr + ldc * 3 + 3) = alpha * _mm256_sumall_epi32(re[15]) + beta * *(C_ptr + ldc * 3 + 3);
			}

			//adddot4x2(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot4x2(padK, alpha,  packedA_ptr + padK * 4, packedB_ptr, beta, C_ptr + 4 * ldc, ldc);
			inline void adddot8x2(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[16] = { _mm256_setzero_si256() };
				__m128i a[8];
				...
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					...
					re[2] = _mm256_madd_epi16_epi32(a[1], b[0], re[2]);
					re[3] = _mm256_madd_epi16_epi32(a[1], b[1], re[3]);
					re[4] = _mm256_madd_epi16_epi32(a[2], b[0], re[4]);
					re[5] = _mm256_madd_epi16_epi32(a[2], b[1], re[5]);
					re[6] = _mm256_madd_epi16_epi32(a[3], b[0], re[6]);
					re[7] = _mm256_madd_epi16_epi32(a[3], b[1], re[7]);
					re[8] = _mm256_madd_epi16_epi32(a[4], b[0], re[8]);
					re[9] = _mm256_madd_epi16_epi32(a[4], b[1], re[9]);
					re[10] = _mm256_madd_epi16_epi32(a[5], b[0], re[10]);
					re[11] = _mm256_madd_epi16_epi32(a[5], b[1], re[11]);
					re[12] = _mm256_madd_epi16_epi32(a[6], b[0], re[12]);
					re[13] = _mm256_madd_epi16_epi32(a[6], b[1], re[13]);
					re[14] = _mm256_madd_epi16_epi32(a[7], b[0], re[14]);
					re[15] = _mm256_madd_epi16_epi32(a[7], b[1], re[15]);
				}
				*C_ptr = alpha * _mm256_sumall_epi32(re[0]) + beta * *C_ptr;
				*(C_ptr + 1) = alpha * _mm256_sumall_epi32(re[1]) + beta * *(C_ptr + 1);
				*(C_ptr + ldc) = alpha * _mm256_sumall_epi32(re[2]) + beta * *(C_ptr + ldc);
				*(C_ptr + ldc + 1) = alpha * _mm256_sumall_epi32(re[3]) + beta * *(C_ptr + ldc + 1);
				*(C_ptr + ldc * 2) = alpha * _mm256_sumall_epi32(re[4]) + beta * *(C_ptr + ldc * 2);
				*(C_ptr + ldc * 2 + 1) = alpha * _mm256_sumall_epi32(re[5]) + beta * *(C_ptr + ldc * 2 + 1);
				*(C_ptr + ldc * 3) = alpha * _mm256_sumall_epi32(re[6]) + beta * *(C_ptr + ldc * 3);
				*(C_ptr + ldc * 3 + 1) = alpha * _mm256_sumall_epi32(re[7]) + beta * *(C_ptr + ldc * 3 + 1);
				*(C_ptr + ldc * 4) = alpha * _mm256_sumall_epi32(re[8]) + beta * *(C_ptr + ldc * 4);
				*(C_ptr + ldc * 4 + 1) = alpha * _mm256_sumall_epi32(re[9]) + beta * *(C_ptr + ldc * 4 + 1);
				*(C_ptr + ldc * 5) = alpha * _mm256_sumall_epi32(re[10]) + beta * *(C_ptr + ldc * 5);
				*(C_ptr + ldc * 5 + 1) = alpha * _mm256_sumall_epi32(re[11]) + beta * *(C_ptr + ldc * 5 + 1);
				*(C_ptr + ldc * 6) = alpha * _mm256_sumall_epi32(re[12]) + beta * *(C_ptr + ldc * 6);
				*(C_ptr + ldc * 6 + 1) = alpha * _mm256_sumall_epi32(re[13]) + beta * *(C_ptr + ldc * 6 + 1);
				*(C_ptr + ldc * 7) = alpha * _mm256_sumall_epi32(re[14]) + beta * *(C_ptr + ldc * 7);
				*(C_ptr + ldc * 7 + 1) = alpha * _mm256_sumall_epi32(re[15]) + beta * *(C_ptr + ldc * 7 + 1);
			}

			//adddot8x1(padK, alpha,  packedA_ptr, packedB_ptr, beta, C_ptr, ldc);
			//adddot8x1(padK, alpha,  packedA_ptr + padK * 8, packedB_ptr, beta, C_ptr + 8 * ldc, ldc);
			inline void adddot16x1(const int padK, const float alpha, const signed char* packedA_ptr, const signed char* packedB_ptr,
				const float beta, int* C_ptr, const int ldc)
			{
				__m256i re[16] = { _mm256_setzero_si256() };
				__m128i a[16];
				...
				for (int j = 0; j < padK; j += mm_align_size2)
				{
					...
					a[6] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[6] + j));
					a[7] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[7] + j));
					a[8] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[8] + j));
					a[9] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[9] + j));
					a[10] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[10] + j));
					a[11] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[11] + j));
					a[12] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[12] + j));
					a[13] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[13] + j));
					a[14] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[14] + j));
					a[15] = _mm_load_si128((__m128i*)(packedA_ptr + padK_offset[15] + j));
					b = _mm_load_si128((__m128i*)(packedB_ptr + j));
					re[0] = _mm256_madd_epi16_epi32(a[0], b, re[0]);
					re[1] = _mm256_madd_epi16_epi32(a[1], b, re[1]);
					re[2] = _mm256_madd_epi16_epi32(a[2], b, re[2]);
					...
					re[6] = _mm256_madd_epi16_epi32(a[6], b, re[6]);
					re[7] = _mm256_madd_epi16_epi32(a[7], b, re[7]);
					re[8] = _mm256_madd_epi16_epi32(a[8], b, re[8]);
					re[9] = _mm256_madd_epi16_epi32(a[9], b, re[9]);
					re[10] = _mm256_madd_epi16_epi32(a[10], b, re[10]);
					re[11] = _mm256_madd_epi16_epi32(a[11], b, re[11]);
					re[12] = _mm256_madd_epi16_epi32(a[12], b, re[12]);
					re[13] = _mm256_madd_epi16_epi32(a[13], b, re[13]);
					re[14] = _mm256_madd_epi16_epi32(a[14], b, re[14]);
					re[15] = _mm256_madd_epi16_epi32(a[15], b, re[15]);
				}
				for (int i = 0; i < 16; i++)
				{
					*(C_ptr + ldc * i) = alpha * _mm256_sumall_epi32(re[i]) + beta * *(C_ptr + ldc * i);
			...
			void execution_kernel_16register(const int M, const int N, const int padK, const float alpha, signed char* packedA,
				signed char* packedB, const float beta, int* C, const int ldc)
			{
				const int quotient_M = M / 4;
				const int quotient_N = N / 4;
				const int remain_M = M % 4;
