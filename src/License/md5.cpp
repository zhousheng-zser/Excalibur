...
#define II(a,b,c,d,x,s,ac) \
		...
		void MD5Init(md5_digest_context *context);
		void MD5Update(md5_digest_context* context, const uint8_t* data, uint32_t size);
		void MD5Encode(uint8_t *output, uint32_t *input, uint32_t len);
		void MD5Decode(uint32_t *output, const uint8_t *input, uint32_t len);
		void MD5Final(md5_digest_context *context);
		...
		void MD5Update(md5_digest_context* context, const uint8_t* data, uint32_t size)
		{
			...
			memcpy(&context->buffer[index], &data[i], size - i);
		}
		...
		void MD5Final(md5_digest_context *context)
		{
			...
			MD5Encode(context->digest, context->state, 16);
		}
        ...
        md5_init_context_ptr md5_init_context = MD5Init;
        md5_final_context_ptr md5_final_context = MD5Final;
        md5_update_context_ptr md5_update_context = MD5Update;
#endif
	}
}
