...
namespace
{
	std::atomic<glasssix::hippogriff::unauthorized_handler_type> unauthorized_handler = nullptr;
}
...
namespace glasssix
{
	...
	{
		extern unsigned long license_thread_id;

		/// <summary>
		/// Check if a license is valid and terminate the process when failed.
		/// </summary>
		void check_license_fatal_exit_sync()
		{
			if (license_thread_id != 0)
			{
#ifdef _MSC_VER
				PostThreadMessage(license_thread_id, WM_ACTIVATE, 0, 0);
#endif
			}
		}

		void set_unauthorized_handler(unauthorized_handler_type handler)
		{
		...
		{
			if (!check_license(component_name))
			{
				/*common::fatal_exit();*/

				unauthorized_handler_type current_handler = unauthorized_handler;
