...
namespace glasssix
{
	namespace memory
	{
        ...
        const size_t GPUMemory::Manager::MAX_CACHED_SIZE = 1 << MAX_BIN;  // 4M
        ...
        bool GPUMemory::Workspace::safe_reserve(size_t size, int device)
		{
            ...
            getinfo(&gpu_bytes_left, &total_memory, true);
        ...
        bool GPUMemory::Workspace::try_reserve(size_t size, int device)
		{
            ...
            if (size > 0UL && (size > size_ || ptr_ == nullptr)) 
            {
                ...
                //TODO: pstream_ = Excalibur::thread_pstream(0);
        ...
        void GPUMemory::Manager::init(const std::vector<int>& gpus, bool debug)
		{
            if (initialized_) 
            {
                return;
            }
            const int count = get_cuda_device_count();
        ...
        void GPUMemory::Manager::lazy_init(int device)
		{
            ...
            LOG(WARNING) << "Lazily initializing GPU Memory Manager Scope on device " << device
                << ". Note: it's recommended to do this explicitly in your main() function.";
            std::vector<int> gpus(1, device);
            static Scope gpu_memory_scope(gpus);
        }

        bool GPUMemory::Manager::try_allocate(void** ptr, size_t size, int device,
            const std::shared_ptr<CUDAStream>& pstream)
		{
            if (!initialized_) 
            {
                lazy_init(device);
            ...
            CHECK_NOTNULL(ptr);
            /*CHECK_EQ(Excalibur::current_device(), device);
            CHECK_EQ(Excalibur::device(), device);*/
