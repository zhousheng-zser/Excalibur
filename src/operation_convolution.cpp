...
namespace glasssix
{
	namespace excalibur
	{
			...
			std::vector<std::shared_ptr<memory::tensor<float>>>& tops)
		{
			...
			{
				return forward_cpu_int8(bottoms, tops);
			}
			...
			if (pad_left_ != 0) {
				make_border<float>(bottoms[0], border_bottom, pad_top_, pad_bottom_, pad_left_, pad_right_, border_constant, pad_value_);
			}
			else {
				border_bottom = bottoms[0];
			}

			if ((kernel_size_h_ == 3 && kernel_size_w_ == 3) && (stride_h_ == 1 && stride_w_ == 1) && output_channel_ < 128)
			{
				conv3x3s1_winograd23_tr_kernel(weights_f32_[0], kernel_tm, input_channel_, output_channel_);
				conv3x3s1_winograd23(border_bottom, tops[0], kernel_tm, true);
			}
			else
			{
				forward_im2col_tr_kernel(kernel_tm);
				forward_im2col(border_bottom, tops[0], kernel_tm, weights_f32_[1], kernel_size_h_, kernel_size_w_, stride_h_, stride_w_, true);
			}
			 
		}

		template<typename Dtype>
		void operation_convolution<Dtype>::forward_cpu_int8(const std::vector<std::shared_ptr<memory::tensor<float>>>& bottoms,
			std::vector<std::shared_ptr<memory::tensor<float>>>& tops)
		{
			CHECK_EQ(bottoms.size(), 1);
			CHECK_EQ(tops.size(), 1);
			...
			{
				tops[0].reset(new memory::tensor<float>(std::vector<int>{num_, output_channel_, output_dim_h_, output_dim_w_}, params_.device_, order, nullptr));
				top_int32.reset(new memory::tensor<int>(std::vector<int>{num_, output_channel_, output_dim_h_, output_dim_w_}, params_.device_, order, nullptr));
			}
			else if (order == memory::NHWC)
			{
				tops[0].reset(new memory::tensor<float>(std::vector<int>{num_, output_dim_h_, output_dim_w_, output_channel_}, params_.device_, order, nullptr));
			...
			if (pad_left_ != 0 || pad_right_ != 0 || pad_top_ != 0 || pad_bottom_ != 0) {
				make_border<signed char>(bottom_int8_, bottom_int8_bordered, pad_top_, pad_bottom_, pad_left_, pad_right_, border_constant, pad_value_);
			}
			else {
				bottom_int8_bordered = bottom_int8_;
			}
			if ((kernel_size_h_ == 3 && kernel_size_w_ == 3) && (stride_h_ == 1 && stride_w_ == 1) && output_channel_ < 128)
			{
				// winograd is slow on small channel count
				use_winograd3x3_int8 = true;
				conv3x3s1_winograd23_tr_kernel_int8(weights_i8_[0], kernel_tm_int8, input_channel_, output_channel_);
				conv3x3s1_winograd23_int8_(bottom_int8_bordered, top_int32, kernel_tm_int8,true);
				...
				dequantize_int32_to_float32(top_int32, tops[0],scale_in, bias_term_ ? (const float*)bias_data : 0, output_channel_);
			}
			else
			{
				...
				conv_im2col_sgemm_int8_dequant_sse(bottom_int8_bordered, tops[0], weights_i8_[0], weights_f32_[1], kernel_size_h_, kernel_size_w_, stride_h_, stride_w_, dequantize_scales, true);
			}
		}

		template<typename Dtype>
		void operation_convolution<Dtype>::conv3x3s1_winograd23_tr_kernel(const std::shared_ptr<memory::tensor<float> >& kernel,
			std::shared_ptr<memory::tensor<float> >& kernel_tm, int inch, int ou)
		{
			kernel_tm.reset(new memory::tensor<float>(std::vector<int>{1, ou, inch, 4 * 4}));
...
#pragma omp parallel for num_threads(2) 
		...
		template<typename Dtype>
		void operation_convolution<Dtype>::conv3x3s1_winograd23(const std::shared_ptr<memory::tensor<float> >& bottom_blob, std::shared_ptr<memory::tensor<float> >& top_blob, 
			const std::shared_ptr<memory::tensor<float> >& kernel_tm, bool bias_term_)
		{
			...
			top_blob.reset(new memory::tensor<float>(std::vector<int>{num, output_channel_, h - 2, w - 2}, -1, memory::NCHW, nullptr));
			...
			make_border<float>(bottom_blob, bottom_blob_bordered, 0, h - bottom_blob->height(), 0, w - bottom_blob->width(), border_constant, 0);
			...
			top_blob_bordered.reset(new memory::tensor<float>(std::vector<int>{num, ou, outh, outw}));
...
#pragma omp parallel for num_threads(2) 
				...
				std::shared_ptr<memory::tensor<float> > top_blob_tm;
				{
					...
					int nRowBlocks = w_tm >> 2;
					const int tiles = nColBlocks * nRowBlocks;
					top_blob_tm.reset(new memory::tensor<float>(std::vector<int>{1, ou, tiles, 16}));
...
#pragma omp parallel for num_threads(2) 
			...
			operation_convolution<float>::cut_border_cpu(top_blob_bordered, top_blob, 0, top_blob_bordered->height() - top_blob->height(), 0, top_blob_bordered->width() - top_blob->width());
		}

		template<typename Dtype>
		void operation_convolution<Dtype>::forward_im2col_tr_kernel(std::shared_ptr<memory::tensor<float> >& kernel_tm)
		{
			...
			kernel_tm.reset(new memory::tensor<float>(std::vector<int>{ 1, outch / 8 + (outch % 8) / 4 + outch % 4, inch, 8 * kernel_size}, params_.device_, memory::NCHW, nullptr));
			int nn_outch = 0;
...
#pragma omp parallel for num_threads(2) 
		...
		template<typename Dtype>
		void operation_convolution<Dtype>::forward_im2col(const std::shared_ptr<memory::tensor<float> >& bottom_blob, std::shared_ptr<memory::tensor<float> >& top_blob, const std::shared_ptr<memory::tensor<float> >& kernel_tm, 
			const std::shared_ptr<memory::tensor<float> >& _bias,const int kernel_w, const int kernel_h, const int stride_w, const int stride_h, bool bias_term_)
		{
			int w = bottom_blob->width();
			int h = bottom_blob->height();
			int inch = bottom_blob->channels();
			...
			// im2col
			bottom_im2col.reset(new memory::tensor<float>(std::vector<int>{1, 1, kernel_size* inch, out_size}, params_.device_, memory::NCHW, nullptr));
			...
			bottom_tm.reset(new memory::tensor<float>(std::vector<int>{1, out_size / 8 + out_size % 8, inch, 8 * kernel_size}, params_.device_, memory::NCHW, nullptr));
			...
			int stride = kernel_h * kernel_w * outw * outh;
...
#pragma omp parallel for num_threads(2) 
		...
		template <typename Dtype>
		void operation_convolution<Dtype>::conv3x3s1_winograd23_tr_kernel_int8(const std::shared_ptr<memory::tensor<signed char> >& kernel,
			std::shared_ptr<memory::tensor<short> >& kernel_tm, int inch, int ou)
		{
			kernel_tm.reset(new memory::tensor<short>(std::vector<int>{1, ou, inch, 4 * 4}));
			const signed char* kernel_data = kernel->cpu_data();
...
#pragma omp parallel for num_threads(2) 
		...
		template <typename Dtype>
		void operation_convolution<Dtype>::conv3x3s1_winograd23_int8_(const std::shared_ptr<memory::tensor<signed char> >& bottom_blob, 
			std::shared_ptr<memory::tensor<int> >& top_blob,const std::shared_ptr<memory::tensor<short> >& kernel_tm, bool bias_term_) 
		{
			int num = bottom_blob->num();
			int w = bottom_blob->width();
			int h = bottom_blob->height();
			...
			const short* kernel_tm_data = kernel_tm->cpu_data();
			const float* bias = nullptr;
			if (bias_term_)
			{
				bias = weights_f32_[1]->cpu_data();
			...
			for (int n = 0; n < num; n++)
			{
...

				{
					...
					const int tiles = nColBlocks * nRowBlocks;
					bottom_blob_int8_tm.reset(new memory::tensor<short>(std::vector<int>{1, inch, tiles, 16}));
					short* bottom_blob_tm_data = bottom_blob_int8_tm->mutable_cpu_data();
	...
	#pragma omp parallel for num_threads(2) 
			...
			//top_blob = top_blob_bordered;
			operation_convolution<int>::cut_border_cpu(top_blob_int8_bordered, top_blob, 0, top_blob_int8_bordered->height() - top_blob->height(), 0, top_blob_int8_bordered->width() - top_blob->width());
		}

		template <typename Dtype>
		void operation_convolution<Dtype>::conv_im2col_sgemm_int8_dequant_sse(const std::shared_ptr<memory::tensor<signed char> >& bottom_blob, std::shared_ptr<memory::tensor<float> >& top_blob, std::shared_ptr<memory::tensor<signed char> >& _kernel,
			const std::shared_ptr<memory::tensor<float> >& _bias, const int kernel_h, const int kernel_w, const int stride_h, const int stride_w, float scale_dequant ,bool bias_term_)
		{
			int w = bottom_blob->width();
			int h = bottom_blob->height();
			int inch = bottom_blob->channels();
			...
			const signed char* kernel = _kernel->cpu_data();
			const float* bias = nullptr;
			if (bias_term_)
				bias = _bias->cpu_data();
			...
			// im2row
			bottom_im2row.reset(new memory::tensor<signed char>(std::vector<int>{1, 1,out_size,kernel_size* inch}, params_.device_, memory::NCHW, nullptr));
			signed char* ret = bottom_im2row->mutable_cpu_data();
			{
				int retID = 0;
			...
			const signed char* bottom_im2col_data = bottom_im2row->cpu_data();
			int bottom_im2row_w = bottom_im2row->width();
			...
			bottom_tm_int8.reset(new memory::tensor<signed char>(std::vector<int>{1, out_size / 4 + out_size % 4, inch, 4 * kernel_size}, params_.device_, memory::NCHW, nullptr));
			signed char* bottom_tm_data = bottom_tm_int8->mutable_cpu_data();
			int bottom_tm_cstep = bottom_tm_int8->width() * bottom_tm_int8->height();
			{
...
#pragma omp parallel for num_threads(2) 
			...
			kernel_tm_int8_sgemm.reset(new memory::tensor<signed char>(std::vector<int>{1, outch / 4 + outch % 4, inch, 4 * kernel_size}, params_.device_, memory::NCHW, nullptr));
			signed char* kernel_tm_data = kernel_tm_int8_sgemm->mutable_cpu_data();
			int kernel_tm_cstep = kernel_tm_int8_sgemm->width() * kernel_tm_int8_sgemm->height();
			{
				int nn_outch = 0;
				int remain_outch_start = 0;
...
#pragma omp parallel for num_threads(2) 
			...
			std::shared_ptr<memory::tensor<signed char>>& dst, float scale)
		{
			...
			signed char* bottom_int8 = dst->mutable_cpu_data();

#pragma omp parallel for num_threads(2) 
			for (int q = 0; q < channels; q++)
			{
				const float* ptr = bottom + q * size;
				signed char* outptr = bottom_int8 + q * size;
		...
		void operation_convolution<Dtype>::dequantize_int32_to_float32(std::shared_ptr<memory::tensor<int>>& src,
			std::shared_ptr<memory::tensor<float>>& dst, float scale, const float* bias_, int bias_data_size)
		{
...
#pragma omp parallel for num_threads(2) 
				for (int q = 0; q < channels; q++)
				{
					...
					float bias = bias_data_size > 1 ? bias_[q] : bias_[0];
...
#pragma omp parallel for num_threads(2) 
		...
		template <typename Dtype>
		void operation_convolution<Dtype>::cut_border_cpu(const std::shared_ptr<memory::tensor<Dtype>>& src,
		...
		template<typename Dtype>
		void operation_convolution<Dtype>::forward_sbias(float* output, const float* bias, memory::orderType order)
		{
			if (order == memory::NCHW)
			{
		...
		template<typename Dtype>
		void operation_convolution<Dtype>::forward_k1s1_f32(const std::shared_ptr < memory::tensor<float>>& bottom,
...
#pragma omp parallel for num_threads(2) 
			...
			if (bias_term_)
			{
				forward_sbias(top_data, weights_f32_[1]->cpu_data(), memory::NCHW);
			}
		}
